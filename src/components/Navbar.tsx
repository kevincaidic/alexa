    ...
    <>
      <motion.header
        ...
        className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
          scrolled ? "py-4 bg-[#050505]/70 backdrop-blur-md border-b border-white/5" : "py-6 bg-transparent"
        }`}
        id="navbar-header"
      >
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between">
          ...
          <a
            ...
            id="brand-logo-link"
          >
            <span className="text-white group-hover:text-accent-teal transition-colors">
              METHUSHIELA
            </span>
            <span className="text-[10px] uppercase text-white/40 tracking-[0.3em] font-mono font-normal">
              ALEXA CAGAANAN
            </span>
          </a>
              ...
              return (
                <button
                  ...
                  className={`relative px-4 py-1.5 text-xs font-display tracking-wide uppercase transition-colors rounded-full ${
                    isActive ? "text-[#050505] font-semibold" : "text-white/60 hover:text-white"
                  }`}
          ...
          <div className="hidden lg:flex items-center gap-6">
            {/* Live GMT Ticker */}
            <div className="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-md border border-white/5 text-[11px] font-mono text-white/50" id="live-time-ticker">
              <Clock className="w-3 h-3 text-accent-teal animate-pulse" />
          ...
          <div className="flex items-center gap-4 lg:hidden">
            {timeStr && (
              <div className="flex items-center gap-1.5 bg-white/5 px-2 py-1 rounded text-[10px] font-mono text-white/50">
                <Clock className="w-2.5 h-2.5 text-accent-teal" />
                <span>{timeStr}</span>
              </div>
            )}
            <button
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
              className="p-2 text-white/80 hover:text-white bg-white/5 rounded-full border border-white/5"
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 bg-[#050505]/95 backdrop-blur-lg z-45 flex flex-col pt-24 px-6 pb-12 lg:hidden"
            id="mobile-drawer-root"
          >
            <div className="flex flex-col gap-6 text-center my-auto">
              <span className="text-white/40 uppercase font-mono text-[10px] tracking-[0.2em]">Navigation</span>
              {navItems.map((item, idx) => (
                <motion.button
                  ...
                  key={item.name}
                  onClick={() => handleScrollTo(item.href)}
                  className="text-2xl font-display font-medium text-white hover:text-accent-teal transition-colors tracking-wide"
